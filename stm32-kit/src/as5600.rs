...
impl As5600 {
    pub fn new(i2c_mutex: &'static I2cMutex) -> Self {
        As5600 {
    ...
    pub async fn initialize(&mut self) -> Result<(), stm32_i2c::Error> {
        self.set_fast_sampling().await?;
        Timer::after(Duration::from_millis(10)).await;
        self.set_digital_output_mode().await?;
        Timer::after(Duration::from_millis(10)).await;
        Ok(())
    }

    pub async fn read_raw_angle(&mut self) -> Result<u16, stm32_i2c::Error> {
        if let Some(p) = self.i2c_mutex.lock().await.as_mut() {
            let mut buffer: [u8; 2] = [0, 0];
            i2c::read(p, I2C_ADDRESS, REGISTER_RAW_ANGLE_H, &mut buffer).await?;
            let high_byte = (buffer[0] & 0b0000_1111) as u16;
            let low_byte = buffer[1] as u16;
            Ok(high_byte << 8 | low_byte)
        } else {
            Err(stm32_i2c::Error::Bus)
        }
    }

    pub async fn read_magnet_status(&mut self) -> Result<MagnetStatus, stm32_i2c::Error> {
        if let Some(p) = self.i2c_mutex.lock().await.as_mut() {
            let mut buffer: [u8; 1] = [0];
            i2c::read(p, I2C_ADDRESS, REGISTER_STATUS, &mut buffer).await?;
            let byte = &buffer[0];
    ...
    pub async fn set_digital_output_mode(&mut self) -> Result<(), stm32_i2c::Error> {
        if let Some(p) = self.i2c_mutex.lock().await.as_mut() {
            let mut buffer: [u8; 1] = [0];
            i2c::read(p, I2C_ADDRESS, REGISTER_CONF_L, &mut buffer).await?;
            // Set the bit 5,4 to 0b10 (digital output mode)
            buffer[0] &= 0b1100_1111;
            buffer[0] |= 0b0010_0000;
            i2c::write_byte(p, I2C_ADDRESS, REGISTER_CONF_L, buffer[0]).await?;
    ...
    pub async fn set_fast_sampling(&mut self) -> Result<(), stm32_i2c::Error> {
        if let Some(p) = self.i2c_mutex.lock().await.as_mut() {
            let mut buffer: [u8; 1] = [0];
            i2c::read(p, I2C_ADDRESS, REGISTER_CONF_H, &mut buffer).await?;
            // set bit 1~0 to 0b11 (fastest sampling)
            buffer[0] &= 0b1111_1100;
            buffer[0] |= 0b0000_0011;
            i2c::write_byte(p, I2C_ADDRESS, REGISTER_CONF_H, buffer[0]).await?;
