...
`actuator-sysid`는 CAN으로 실기기 모터를 MotionControl 모드로 구동하며 데이터를 수집하고, 같은 실행에서 MuJoCo actuator 파라미터를 least-squares로 맞춥니다. sysid 명령은 `angle_ref`만 바꾸고 `dq_ref=0`, `tau_ff=0`으로 고정합니다. 실기기에 보낼 MotionControl의 `kp/kd`는 `--motion-kp`, `--motion-kd`로 지정합니다. 펌웨어와 시뮬레이터 모두 `kp`는 출력축 기준 `Nm/rad`, `kd`는 출력축 기준 `Nm/(rad/s)`로 해석합니다. 튜닝에서는 캡처에 기록된 `kp/kd`를 그대로 고정하고 나머지 plant 파라미터만 추정합니다. `--amplitude`는 angle reference amplitude[rad]입니다. 안전을 위해 실제 구동은 `--yes`를 명시해야 시작됩니다.
...
튜닝은 stage fitting으로 진행합니다. 전체 동적 구간에서 `delay_s`를 먼저 맞추고, `pulses`/`large_prbs` 구간에서 `torque_limit`을 맞춥니다. 그 다음 `pulses`/`large_prbs` 구간에서 `armature`를 맞춘 뒤, `small_prbs`/`medium_prbs`/`sweep` 구간에서 `damping`과 `frictionloss`를 맞춥니다. 각 stage는 multi-start least-squares를 사용합니다.

튜닝 대상은 `joint damping`, `joint frictionloss`, `joint armature`, `delay_s`, `torque_limit`입니다. `kp/kd`는 캡처에 기록된 값을 그대로 사용합니다. 시뮬레이션 torque는 `tau = kp * (angle_ref - angle) - kd * velocity`로 계산하고 추정된 `torque_limit`으로 clip합니다. residual은 측정 position과 시뮬레이션 position의 차이만 사용합니다.
