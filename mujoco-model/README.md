...
튜닝은 stage fitting으로 진행합니다. 전체 동적 구간에서 `delay_s`를 먼저 맞추고, `pulses`/`large_prbs` 구간에서 `torque_limit`을 맞춥니다. 그 다음 `small_prbs`/`medium_prbs`/`sweep` 구간에서 유효 Motion gain `kp/kd`를 맞추고, `pulses`/`large_prbs` 구간에서 `armature`를 맞춘 뒤, `small_prbs`/`medium_prbs`/`sweep` 구간에서 `damping`과 `frictionloss`를 맞춥니다. 각 stage는 multi-start least-squares를 사용합니다.

튜닝 대상은 `joint damping`, `joint frictionloss`, `joint armature`, `delay_s`, `kp`, `kd`, `torque_limit`입니다. 시뮬레이션 torque는 `tau = kp * (angle_ref - angle) - kd * velocity`로 계산하고 추정된 `torque_limit`으로 clip합니다. residual은 측정 position과 시뮬레이션 position의 차이만 사용합니다.
